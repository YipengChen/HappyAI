...
while cap.isOpened():
    ...
    face_points = face_mesh.get_all_locations(results, image.shape, has_z=True)

    draw_image = image.copy()
    if face_points is not None and len(face_points) == 2:
        
        # change second face to first face
        hull_points = find_convex_hull_points(face_points[0][:, 0:2])
        ...
        for triangle_index in target_triangle_indexs:
            target_triangle_points = face_points[1][:,0:2][triangle_index]
            image_triangle_points = face_points[0][:,0:2][triangle_index]
            image_triangle_z = face_points[0][:,2][triangle_index].mean()
        ...
        for triangle_pair in triangle_pairs:
            warp_triangle(image, draw_image, triangle_pair[0], triangle_pair[1])
            
        draw_image = np.uint8(draw_image * vaild_mask_image + image * invert_vaild_mask_image)

        # change first face to second face
        temp_image = draw_image.copy()
        hull_points = find_convex_hull_points(face_points[1][:, 0:2])
        vaild_mask_image = np.zeros((draw_image.shape), dtype=np.float32)
        cv2.fillConvexPoly(vaild_mask_image, np.int32(hull_points), (1.0, 1.0, 1.0), 16, 0)
        vaild_mask_image = cv2.erode(vaild_mask_image, np.ones((5, 5), dtype=np.uint8), iterations=1)
        vaild_mask_image = cv2.GaussianBlur(vaild_mask_image, (21, 21), 100, 100)
        ...
        triangle_pairs = []
        for triangle_index in target_triangle_indexs:
            target_triangle_points = face_points[0][:,0:2][triangle_index]
            image_triangle_points = face_points[1][:,0:2][triangle_index]
            image_triangle_z = face_points[1][:,2][triangle_index].mean()
            triangle_pairs.append([target_triangle_points, image_triangle_points, image_triangle_z])
        ...
        draw_image = np.uint8(draw_image * vaild_mask_image + temp_image * invert_vaild_mask_image)
